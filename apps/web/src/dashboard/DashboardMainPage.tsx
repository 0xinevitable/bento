import { BentoUser, BentoUserResponse, Wallet } from '@/types/common';
...
export const getServerSideProps: GetServerSideProps<Props> = async (
  ...
  if (!user) {
    // User not found - no longer creating profiles via Supabase
    return { notFound: true };
  }
...
const DashboardMainPage = (props: Props) => {
  ...
  const wallets = useMemo<Wallet[]>(() => profile.wallets || [], [profile]);

  const hasWallet = !!session && wallets.length > 0;

  const hasLoggedTabViewEvent = useRef<boolean>(false);
  useEffect(() => {
    if (!hasLoggedTabViewEvent.current && !session) {
  ...
  const revalidateWallets = useCallback(async () => {
    const { data } = await axiosWithCredentials.get<BentoUserResponse>(
      `/users/${profile.username || profile.id}`,
    );
    if (data?.result) {
